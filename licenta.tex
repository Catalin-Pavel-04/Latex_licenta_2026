...
Figura \ref{fig:context-web-uvt} ilustrează un exemplu de context web în care informația oficială este disponibilă, dar poate necesita navigare manuală și identificarea paginii potrivite.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{imagini/site_uvt.png}
\caption{Exemplu de context web UVT în care utilizatorul poate avea nevoie de asistență pentru navigare. Sursă: captură realizată de autor de pe site-ul oficial al Universității de Vest din Timișoara}
\label{fig:context-web-uvt}
\end{figure}
...
Cerințele funcționale urmărite în proiect descriu modul în care utilizatorul interacționează cu sistemul și modul în care backend-ul procesează întrebările. Utilizatorul poate selecta facultatea relevantă sau poate lucra în contextul general UVT, apoi poate trimite întrebări în limbaj natural din popup-ul extensiei. Backend-ul transmite întrebarea către Ollama pentru interpretare lingvistică structurată și validează câmpurile \texttt{corrected\_question}, \texttt{intent}, \texttt{keywords} și \texttt{faculty\_hint}, astfel încât rezultatul modelului să fie tratat ca intrare controlată pentru căutare.
...
Figura \ref{fig:arhitectura-finala-uvt-asist} prezintă arhitectura de ansamblu a sistemului și relațiile dintre extensia Chrome, backend, indexul local, Qdrant și modelele Ollama.
...
\label{fig:arhitectura-finala-uvt-asist}
\end{figure}
...
Figura \ref{fig:structura-proiect-uvt-asist} sintetizează organizarea proiectului pe module, evidențiind separarea dintre interfața browserului, backend-ul Flask, componentele RAG și artefactele de evaluare.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{imagini/figuri_lucrare/diagrama_structura_proiect_uvt_asist.png}
\caption{Structura principală a proiectului UVT Asist}
\label{fig:structura-proiect-uvt-asist}
\end{figure}
...
Figura \ref{fig:model-fragment-indexat} descrie modelul unui fragment indexat, câmpurile sale principale și modul în care fragmentul este transformat într-un punct vectorial interogabil.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{imagini/figuri_lucrare/diagrama_model_fragment_indexat.png}
\caption{Modelul unui fragment indexat în UVT Asist}
\label{fig:model-fragment-indexat}
\end{figure}
...
Figura \ref{fig:pipeline-indexare-surse-oficiale} prezintă pipeline-ul de indexare, de la descoperirea și curățarea surselor oficiale până la segmentarea textului și încărcarea reprezentărilor vectoriale.
...
\label{fig:pipeline-indexare-surse-oficiale}
\end{figure}
...
Figura \ref{fig:pipeline-procesare-intrebare} sintetizează etapele de procesare a unei întrebări, de la validarea cererii și interpretarea lingvistică până la retrieval, reranking, calculul încrederii și generarea răspunsului.
...
\label{fig:pipeline-procesare-intrebare}
\end{figure}
...
Figura \ref{fig:popup-extensie-uvt-asist} prezintă interfața popup a extensiei, cu selectorul de facultate și zona în care utilizatorul interacționează cu asistentul.
...
\label{fig:popup-extensie-uvt-asist}
\end{figure}
...
Figura \ref{fig:endpointuri-backend-flask} grupează endpoint-urile backend-ului Flask după rolul lor în aplicație: verificarea stării, încărcarea facultăților, interacțiunea conversațională, indexarea și colectarea feedback-ului.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{imagini/figuri_lucrare/diagrama_endpointuri_backend_flask.png}
\caption{Endpoint-urile principale ale backend-ului Flask}
\label{fig:endpointuri-backend-flask}
\end{figure}
...
Figura \ref{fig:raspuns-surse-confidence} exemplifică modul în care extensia afișează răspunsul, sursele oficiale și nivelul de încredere asociat rezultatului.
...
\label{fig:raspuns-surse-confidence}
\end{figure}
...
Pentru evaluarea automată extinsă, scriptul dedicat setului independent de 1000 de întrebări rulează fiecare caz prin backend-ul local și compară răspunsul cu o rubrică de evaluare, fără a impune reproducerea exactă a unui text ideal. Figura \ref{fig:flux-evaluare-qa1000} sintetizează fluxul evaluatorului automat, de la încărcarea datasetului până la agregarea metricilor și exportul rapoartelor.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{imagini/figuri_lucrare/diagrama_evaluare_qa1000.png}
\caption{Fluxul evaluării automate pe setul independent de 1000 de întrebări}
\label{fig:flux-evaluare-qa1000}
\end{figure}
...
Figura \ref{fig:rezultate-qa100-inainte-dupa} prezintă comparativ rezultatele evaluării Q\&A pe 100 de întrebări, înainte și după optimizările aplicate asupra mecanismelor de retrieval și control al răspunsului.
...
\label{fig:rezultate-qa100-inainte-dupa}
\end{figure}
...
Pentru o verificare mai largă, aplicația a fost evaluată ulterior pe un benchmark independent de 1000 de întrebări. În această evaluare, \texttt{ideal\_answer} este folosit ca rubrică de evaluare, nu ca text care trebuie reprodus exact. Evaluatorul automat analizează răspunsul și metadatele returnate de sistem: sursele, intenția, \texttt{page\_type}, nivelul de încredere, termenii obligatorii din \texttt{required\_terms} și termenii interziși din \texttt{forbidden\_terms}. Această abordare este compatibilă cu perspectiva evaluărilor RAG, unde trebuie analizate împreună răspunsul, sursele recuperate, metadatele și comportamentul sistemului în cazuri fără răspuns sigur \cite{Gan2025RAGEvalSurvey}. Întrebările fără răspuns sigur sunt evaluate separat, deoarece un comportament corect poate însemna refuzul de a formula o afirmație factuală nesusținută.

Tabelul \ref{tab:rezultate-globale-qa1000} sintetizează rezultatele globale ale benchmark-ului independent, incluzând rata de trecere, scorurile agregate, potrivirea surselor și absența erorilor tehnice.
...
\label{tab:rezultate-globale-qa1000}
\end{table}

Figura \ref{fig:qa1000-rata-trecere-categorii} detaliază grafic rata de trecere pentru fiecare categorie de întrebări, astfel încât performanța sistemului să poată fi analizată în funcție de tipul de nevoie informațională și să fie evidențiate rapid diferențele dintre categoriile cu performanță ridicată și cele problematice.
...
\label{fig:qa1000-rata-trecere-categorii}
\end{figure}

Tabelul \ref{tab:latenta-benchmark-independent} prezintă valorile agregate ale latenței observate în benchmark, utile pentru interpretarea performanței operaționale a aplicației în regim local. Dacă notăm cu \(\ell_1,\ell_2,\ldots,\ell_n\) latențele individuale ale celor \(n\) întrebări și cu \(\ell_{(1)} \leq \ell_{(2)} \leq \ldots \leq \ell_{(n)}\) valorile sortate crescător, indicatorii sunt calculați astfel:
\[
\bar{\ell}=\frac{1}{n}\sum_{i=1}^{n}\ell_i,\qquad
\operatorname{mediana}(L)=
\begin{cases}
\ell_{\left(\frac{n+1}{2}\right)}, & n \text{ impar},\\
\frac{\ell_{\left(\frac{n}{2}\right)}+\ell_{\left(\frac{n}{2}+1\right)}}{2}, & n \text{ par},
\end{cases}
\]
\[
P90=\ell_{(\lceil 0.90n\rceil)},\qquad
P95=\ell_{(\lceil 0.95n\rceil)}.
\]
...
\label{tab:latenta-benchmark-independent}
\end{table}

Figura \ref{fig:qa1000-latenta-medie-categorii} completează analiza latenței prin afișarea timpului mediu de răspuns pe categorii, evidențiind diferențele dintre tipurile de întrebări evaluate.
...
\label{fig:qa1000-latenta-medie-categorii}
\end{figure}
...
Figura \ref{fig:qa1000-distributie-generare} prezintă distribuția modurilor de generare, separând răspunsurile produse cu Ollama de cele construite local, limitate sau transformate în cereri de clarificare.
...
\label{fig:qa1000-distributie-generare}
\end{figure}

Figura \ref{fig:qa1000-distributie-retrieval-backend} prezintă distribuția backend-ului de retrieval, indicând ponderea cazurilor procesate prin Qdrant și a celor oprite sau clarificate înainte de căutarea semantică propriu-zisă.
...
\label{fig:qa1000-distributie-retrieval-backend}
\end{figure}
...
Tabelul \ref{tab:genAIOverview} rezumă rolul instrumentelor de inteligență artificială generativă în procesul de lucru, delimitând sprijinul oferit de acestea de responsabilitatea autorului asupra conținutului final.
