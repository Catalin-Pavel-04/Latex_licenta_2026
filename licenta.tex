...
Din acest motiv, UVT Asist nu tratează modelul de limbaj ca sursă unică de adevăr. Modelul este folosit pentru interpretare lingvistică și formulare, iar informația factuală este extrasă din pagini oficiale selectate determinist. Abordarea este apropiată de paradigma Retrieval-Augmented Generation, în care generarea răspunsului este condiționată de context recuperat dintr-o colecție de documente \cite{Lewis2020RAG}. Această decizie permite păstrarea avantajelor interfeței conversaționale, dar reduce dependența de cunoștințele interne ale modelului.

\placeholderfigure
{Captură ecosistem web UVT}
{Se va insera o captură cu site-ul oficial UVT sau cu o pagină de facultate, evidențiind distribuirea informației pe meniuri, anunțuri și pagini administrative. Imaginea trebuie să ilustreze problema de navigare pe care o rezolvă extensia UVT Asist.}
{Exemplu de context web UVT în care utilizatorul poate avea nevoie de asistență pentru navigare}

\section{Scop, obiective și contribuții}
...
La nivel tehnic, proiectul propune un pipeline complet: descoperirea paginilor, extragerea textului, clasificarea paginilor, segmentarea în fragmente, construirea reprezentărilor vectoriale, stocarea în Qdrant, interpretarea lingvistică a întrebării, validarea rezultatului structurat, căutarea semantică, reranking-ul determinist, verificarea live limitată și generarea locală cu Ollama. La nivel de interacțiune, extensia oferă selecția facultății, istoricul conversației, întrebări recente, afișarea surselor și stări clare pentru backend indisponibil sau indexare în desfășurare.
...
Un flux RAG tipic conține colectarea documentelor, segmentarea lor în fragmente, indexarea embeddings-urilor, interpretarea întrebării, recuperarea fragmentelor relevante, filtrarea și generarea răspunsului pe baza contextului selectat. Această schemă nu elimină complet riscurile: dacă etapa de retrieval selectează surse slabe sau dacă rescrierea întrebării schimbă sensul inițial, modelul poate formula un răspuns slab. De aceea, UVT Asist păstrează întrebarea originală, validează interpretarea lingvistică produsă de model și folosește mecanisme deterministe pentru selecția surselor: retrieval Qdrant, backfill lexical, reranking, scor de încredere și verificare live limitată a surselor de top.
...
Pe lângă funcționalități, proiectul impune cerințe nefuncționale importante. Sistemul trebuie să favorizeze sursele oficiale, să evite afirmațiile nesusținute, să funcționeze local, să degradeze controlat atunci când Qdrant sau Ollama nu sunt disponibile și să rămână utilizabil chiar dacă indexarea este în curs. O altă cerință este transparența: utilizatorul trebuie să vadă sursele, iar backend-ul trebuie să atașeze metadate despre încredere, modul de retrieval, verificarea live, modul de generare și întrebarea interpretată.
...
UVT Asist este construit ca sistem local, compus dintr-o extensie Chrome, un backend Flask, un index JSON, o bază vectorială Qdrant și modele Ollama pentru interpretare lingvistică, embeddings și generare. Produsul vizibil pentru utilizator este extensia, iar restul componentelor rulează local și susțin procesarea întrebării.

\placeholderfigure
{Diagramă arhitectură finală UVT Asist}
{Se va insera o diagramă cu extensia Chrome, backend-ul Flask modularizat, rutele din \texttt{backend/api/}, serviciile din \texttt{backend/services/}, componentele RAG din \texttt{backend/rag/}, Qdrant, Ollama, indexul JSON și sursele oficiale UVT. Diagrama trebuie să indice fluxurile principale: întrebarea originală, interpretarea lingvistică prin Ollama, validarea rezultatului, retrieval, reranking, generare și răspuns cu surse.}
{Arhitectura finală a sistemului UVT Asist}
...
Proiectul aplicației este organizat în două zone principale: \texttt{backend/} și \texttt{extension/}. În varianta curentă, backend-ul separă rutele HTTP, serviciile aplicației, configurația, logica RAG și regulile de ranking. Extensia Chrome este structurată similar: comunicarea cu backend-ul, stocarea locală, starea conversației și randarea interfeței sunt mutate în module JavaScript dedicate.
...
\hline
\texttt{backend/services/} & Grupează serviciile de chat, feedback, health, indexare, telemetry, cache și construire a răspunsului. \\
\hline
...
\hline
\texttt{extension/options.html} & Oferă interfața de configurare pentru URL-ul backend-ului local. \\
\hline
\end{tabularx}
...
Fiecare fragment primește metadate: identificator, facultate, tip de pagină, titlu, URL, text și momentul indexării. Documentul rezultat este salvat ca index JSON, iar fragmentele sunt transformate în embeddings cu Ollama și încărcate în Qdrant. Qdrant stochează un punct vectorial pentru fiecare fragment și permite filtrarea după \texttt{faculty\_id} și \texttt{page\_type}, ceea ce reduce riscul ca o întrebare despre orar sau secretariat să primească surse de la altă facultate.

\placeholderfigure
{Diagramă pipeline indexare}
{Se va insera o diagramă cu pașii de indexare: configurarea surselor oficiale, descoperirea URL-urilor, filtrarea domeniilor, descărcarea HTML/PDF/DOCX, extragerea textului, OCR opțional, clasificarea paginilor, segmentarea în fragmente, salvarea indexului JSON, generarea embeddings-urilor Ollama și încărcarea în Qdrant.}
{Pipeline-ul de indexare al surselor oficiale}
...
\begin{algorithmic}[1]
\Require întrebare originală, facultate selectată, istoric conversațional
\Ensure răspuns, surse oficiale, scor de încredere
\State păstrează întrebarea originală și validează payload-ul JSON
\State aplică normalizare tehnică minimă și safety guards
\State cere de la Ollama \texttt{corrected\_question}, \texttt{intent}, \texttt{keywords}, \texttt{faculty\_hint}
\If{răspunsul Ollama este indisponibil sau invalid}
    \State folosește întrebarea originală și fallback raw
\Else
    \State validează schema, tipurile, lungimile și intenția returnată
\EndIf
\State determină facultatea efectivă fără a suprascrie selecția explicită a utilizatorului
\If{întrebarea necesită clarificarea facultății}
    \State returnează mesaj de clarificare
\EndIf
\If{embedding-ul nu este disponibil}
    \State caută lexical în indexul JSON folosind întrebarea originală sau validată
\Else
...
\placeholderfigure
{Diagramă pipeline interogare}
{Se va insera o diagramă pentru fluxul întrebare originală -- interpretare Ollama -- validare backend -- retrieval -- reranking -- răspuns: păstrarea întrebării originale, \texttt{corrected\_question}, \texttt{intent}, \texttt{keywords}, \texttt{faculty\_hint}, fallback raw fără Ollama, embedding, căutare Qdrant, backfill lexical, scorare în \texttt{backend/rag/ranking/}, verificare live opțională, calcul confidence și generare sau răspuns determinist.}
{Pipeline-ul de procesare a unei întrebări în UVT Asist}
...
\placeholderfigure
{Captură popup extensie Chrome}
{Se va insera o captură cu popup-ul extensiei în stare normală: selectorul de facultate, câmpul de întrebare, starea backend-ului, istoricul conversației și zona de afișare a surselor. Captura trebuie făcută cu backend-ul local disponibil și indexul pregătit.}
{Popup-ul extensiei UVT Asist în stare normală}
...
\placeholderfigure
{Captură răspuns cu surse și confidence}
{Se va insera o captură cu un răspuns complet în popup: textul răspunsului, lista de surse oficiale, nivelul de încredere și badge-ul de proveniență a dovezilor, de exemplu \textit{Index local} sau \textit{Verificat live}. Captura trebuie să arate că utilizatorul poate verifica sursele afișate.}
{Afișarea răspunsului, surselor oficiale și nivelului de încredere}
...
\section{Evaluare independentă pe 1000 de întrebări}
Înaintea benchmark-ului extins, proiectul a fost evaluat pe setul Q\&A de 100 de întrebări definit intern. Fiecare întrebare a fost trimisă către endpoint-ul de chat, iar evaluatorul a verificat scorul Q\&A, sursele returnate, potrivirea nivelului de încredere, comportamentul pentru întrebări fără răspuns sigur și latența. Raportarea compară rezultatele înainte și după optimizările aplicate asupra selecției surselor, reranking-ului, tratării întrebărilor vagi și controlului generării.
...
\placeholderfigure
{Grafic rezultate înainte vs. după optimizări}
{Se va insera un grafic sau un tabel vizual care compară indicatorii principali înainte și după optimizări: rata de trecere, scor mediu Q\&A, scor median, Top-1 URL corect, Top-3 URL corect, potrivirea nivelului de încredere, tratarea întrebărilor fără răspuns sigur și latența. Valorile trebuie să fie cele din Tabelul anterior.}
{Rezultatele evaluării înainte și după optimizări}
...
Latența arată limitele rulării locale. Întrebările pot deveni lente atunci când activează pași suplimentari: interpretarea lingvistică prin Ollama, verificarea live a surselor, extragerea sau consultarea unor documente PDF, calculul embeddings-urilor locale, interogarea Qdrant sau generarea răspunsului cu Ollama. Valorile P90 și P95 arată că o parte dintre întrebări depășesc semnificativ latența mediană, ceea ce este relevant pentru experiența utilizatorului.
...
La nivel funcțional, sistemul oferă selecția facultății, istoric conversațional, întrebări recente, afișarea surselor oficiale, nivel de încredere, verificare live limitată și feedback. La nivel de siguranță, aplicația poate refuza sau limita răspunsul atunci când dovezile oficiale sunt slabe, evitând formularea unor afirmații speculative.
