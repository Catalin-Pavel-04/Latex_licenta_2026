...
{Se va insera o diagramă cu extensia Chrome, backend-ul Flask modularizat, rutele din \texttt{backend/api/}, serviciile din \texttt{backend/services/}, componentele RAG din \texttt{backend/rag/}, Qdrant, Ollama, indexul JSON și sursele oficiale UVT. Diagrama trebuie să indice fluxurile principale: întrebare, retrieval, reranking, generare și răspuns cu surse.}
...
{Se va insera o diagramă pentru fluxul întrebare -- analiză -- retrieval -- reranking -- răspuns: normalizare, detecție intenție, inferență facultate, embedding, căutare Qdrant, backfill lexical, scorare în \texttt{backend/rag/ranking/}, verificare live opțională, calcul confidence și generare sau răspuns determinist.}
...
Proiectul aplicației este organizat în două zone principale: \texttt{backend/} și \texttt{extension/}. În varianta curentă, backend-ul nu mai concentrează toată logica în fișierul Flask principal, ci separă rutele HTTP, serviciile aplicației, configurația, logica RAG și regulile de ranking. Această separare face mai clare responsabilitățile fiecărui modul și permite testarea mai precisă a componentelor critice.

Extensia Chrome este structurată similar: comunicarea cu backend-ul, stocarea locală, starea conversației și randarea interfeței sunt mutate în module JavaScript dedicate. Popup-ul rămâne interfața principală pentru utilizator, iar pagina de opțiuni permite configurarea URL-ului backend-ului local fără modificarea codului.
...
\hline
\texttt{backend/rag/} & Conține analiza întrebării, normalizarea textului, detecția intenției, serviciul de retrieval și calculul nivelului de încredere. \\
\hline
...
\hline
\texttt{backend/tests/} & Conține teste unitare și de integrare pentru API, retriever, confidence, query analysis, caching și cazuri defensive. \\
\hline
...
\hline
\texttt{extension/options.html} și \newline \texttt{extension/options.js} & Oferă interfața de configurare pentru URL-ul backend-ului local. \\
\hline
\end{tabularx}
...
\section{Evaluare cantitativă pe setul Q\&A de 100 de întrebări}
...
{Se va insera un grafic sau un tabel vizual care compară indicatorii principali înainte și după optimizări: pass rate, scor mediu Q\&A, scor median, Top-1 URL corect, Top-3 URL corect, potrivirea nivelului de încredere, tratarea întrebărilor fără răspuns sigur și latența. Valorile trebuie să fie cele din Tabelul anterior.}
